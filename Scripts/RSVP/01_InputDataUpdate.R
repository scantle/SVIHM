...
tribs <- get_tributary_flows(end_date = model_end_date, fj_update = fjd_model, monthly = T, one_regression = T)
...
tribs_regressed = write_trib_file_for_partitioning(gauges = tribs, output_dir = update_dir,filename = 'daily_streamflow_records_regressed.txt',
                           start_date=model_start_date, end_date=model_end_date, monthly = T,
                           old_tribs_df = NA
                           )
