...
curt25_fields <- read_sf(file.path(data_dir["ref_data_dir","loc"],"2025_LCS_Fields_Clean.shp"))

# Reproject to a common CRS (NAD83(2011) / California Albers)
curt25_fields <- st_transform(curt25_fields, crs = st_crs(swbm_fields))
#-------------------------------------------------------------------------------------------------#
...
for (i in 1:length(lcs_type)) {
  ...
  lcs_fields <- curt25_fields[curt25_fields$lcs_type == lcs_type[i], ]
...
for (i in 1:length(lcs_abbr)) {
  for (j in seq_along(months)) {
    ...
    curtailment_df[curtailment_df$Stress_Period == as.Date(paste0("2025-", j+3, "-01")), matching_cols] <-
      curtailment_df[curtailment_df$Stress_Period == as.Date(paste0("2025-", j+3, "-01")), matching_cols] +
...
for (j in seq_along(months)) {
  ...
  etcor_df[etcor_df$Stress_Period == as.Date(paste0("2025-", j+3, "-01")), matching_cols] <-
    etcor_df[etcor_df$Stress_Period == as.Date(paste0("2025-", j+3, "-01")), matching_cols] - swbm_fields[[month_col]]
...
#write_sf(swbm_fields, file.path('C:/Users/lelan/Box/Research/Scott Valley/Analyses/LCS 2025 Plan','swbm_fields_curtail2025.shp'))

#-- Write curtailment as a csv file
write.csv(curtailment_df, file=file.path(data_dir["ref_data_dir","loc"], 'Curtail_25.csv'), row.names = F)

#-- Write ET correction as a csv file
write.csv(etcor_df, file=file.path(data_dir["ref_data_dir","loc"], 'ETcor_25.csv'), row.names = F)
