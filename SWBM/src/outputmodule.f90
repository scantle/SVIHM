...
! ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
    subroutine ET_out_MODFLOW(im,imonth,ndays,nmonth, nrows,ncols,output_zone_matrix,Total_Ref_ET, &
      ET_Zone_Cells, ET_Cells_ex_depth, npoly)
 
    INTEGER, INTENT(IN) :: im,imonth,nmonth,nrows,ncols, npoly
    INTEGER, INTENT(IN) :: output_zone_matrix(nrows,ncols), ndays(nmonth), ET_Zone_Cells(nrows,ncols)
    REAL, INTENT(IN) :: Total_Ref_ET
    REAL :: Avg_Ref_ET
    REAL, DIMENSION(npoly) :: ET_fraction
    INTEGER :: ip
    REAL, DIMENSION(nrows,ncols) ::  ET_matrix_out, ET_Cells_ex_depth(nrows,ncols)
    ...
    do ip=1,npoly
      ...
      where (output_zone_matrix(:,:) == ip)
        ...
        !Extinction_depth_matrix(:,:) = ET_Cells_ex_depth(:,:) ! don't need this
      end where
    end do
    
    ET_matrix_out = ET_matrix_out * ET_Zone_Cells  ! format excluding all cells outside Discharge Zone
    !where (ET_Cells_ex_depth(:,:) == 0)
